...
public class DanceClicked : MonoBehaviour {
	...
	void MoveSquareUp(GameObject square, int c){
		...
		for(int i = 0; i < characters.Length; i++){
			if(square.transform.position == characters[i].transform.position && !characters[i].GetComponent<Movement>().dead){
				if(characters[i].gameObject.tag == "Ally" && transform.parent.gameObject.tag == "Ally" && characters[i].GetComponent<Movement>().turnEnd) index++;
				if(characters[i].gameObject.tag == "Enemy" && transform.parent.gameObject.tag == "Enemy" && characters[i].GetComponent<Movement>().turnEnd) index++;
			}
		}
